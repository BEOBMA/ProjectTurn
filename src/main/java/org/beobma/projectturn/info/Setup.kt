...
class Setup {
    companion object {
        ...
        val cardPackList = listOf(KnightOfFlame, CogsAndMachines, BirthOfTheUniverse, OurShield)
        ...
        val eventList: MutableList<GameEvent> = mutableListOf(
            GameEvent(
                ...
                "저금통", listOf(
                    "${ChatColor.GRAY}모든 플레이어는 이하의 선택지 중 하나를 선택해야 한다.",
                    "${ChatColor.BOLD}저금한다. ${ChatColor.RESET}${ChatColor.GRAY}- 다음번 이 이벤트가 발생하면 유물 보상을 얻는다.",
